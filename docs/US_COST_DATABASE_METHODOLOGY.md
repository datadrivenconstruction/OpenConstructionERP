...
├── vector_adapter.py   # LanceDB vector search adapter
...
|----------|--------|-----------|
| Rate storage | `String(50)` | SQLite float precision; parsed to float on read |
...
```json
{
  ...
  "code": "LAB-2061"            // Optional: link to a separate resource catalog entry
}
```
...
```bash
# Update the import script's DATA_FILE pointer
...
The pilot includes `scripts/import_cost_database.py` which:

1. Logs in via demo auth
2. Loads the JSON data file
3. Validates `rate == sum(components.cost)` for each item
4. Calls `POST /costs/bulk/`
5. Verifies a sample of items by searching the API
...
curl -s http://localhost:8082/api/v1/costs/catalogues/ | python3 -m json.tool

# Load a catalog
curl -X POST http://localhost:8082/api/v1/costs/load-cwicr/DE_BERLIN \
  -H "Authorization: Bearer $TOKEN"
```
...
```bash
curl -X DELETE http://localhost:8082/api/v1/costs/actions/clear-region/USA_TENNESSEE \
  -H "Authorization: Bearer $TOKEN"
```
...
OCERP uses `COUNTRYCODE_CITY` or `COUNTRYCODE_STATE` format:
...
DE_BERLIN         # Berlin, Germany (CWICR convention)
```

For US regions, prefer state-level identifiers (`USA_TENNESSEE`) over city identifiers (`USA_NASHVILLE`) unless the data is metro-specific (e.g., BLS MSA data).
...
│   └── us_tn_sitework_costs.json     # Final CostItem array (Sec 4)
├── scripts/
│   └── import_cost_database.py        # Reusable import script (Sec 12.3)
├── docs/
