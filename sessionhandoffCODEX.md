...
## Current closeout — 2026-08-25

- The smallest login branding fix is implemented in `frontend/src/features/auth/LoginPage.tsx` using the existing `usePartnerPack` and `partnerLogoUrl` path. Active partner branding takes priority; the prior default and custom-branding branches remain intact when no pack is active.
- Focused regression coverage is in `frontend/src/features/auth/LoginPage.partner-branding.test.tsx`; Vitest passed 1 file and 1 test. The prior frontend typecheck and production build also passed.
- Standalone Playwright UAT passed for the active Mullets login branding, existing Admin demo sign-in, and dashboard load. Evidence is under `output/playwright/uat-login-branding-2026-08-25/`.
- The generated `.playwright-cli/` artifacts were moved, without deletion, to `DELETED/.playwright-cli/` per project rule.
- The live application container is healthy at `http://127.0.0.1:18080`; PostgreSQL, SQL Server, and named volumes were left running and untouched by this source change.
- `continue.txt` is intentionally excluded from this commit and must not be edited. Review the staged set for secrets before publication.
- This closeout is the current bounded slice. The next action is review, commit, and push of the intended files on branch `001`.
