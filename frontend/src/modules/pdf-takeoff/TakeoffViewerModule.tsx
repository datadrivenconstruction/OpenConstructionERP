...
export default function TakeoffViewerModule({
  ...
  const handleCalibrationConfirm = useCallback(
    (nextScale: ScaleConfig) => {
      ...
      const unit = nextScale.unitLabel as 'm' | 'mm' | 'ft' | 'in';
      const realLength = calibrationPixels > 0 ? calibrationPixels / nextScale.pixelsPerUnit : 0;
      setLastCalibration({ realLength, unit });
      addToast({
        type: 'success',
        title: t('takeoff_viewer.calibrated', { defaultValue: 'Scale calibrated' }),
        message: `${formatScaleRatio(nextScale)} · ${realLength.toFixed(2)} ${unit}`,
      });
    },
